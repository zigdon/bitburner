...
export function int(n, digits=0) {
    return commaFmt0.format(n.toFixed(digits));
}

/**
 * @param {number} n
 * @param {number} digits
 * @param {boolean} fmtstring
 * @returns {string}
 */
 ...
 * @returns {string}
 */
export function large(n, {suffix= ["", "k", "m", "b", "t", "q"], digits=2} = {}) {
...
export function money(n, digits=0) {
  return "$"+large(Number(n), {digits:2});
}
