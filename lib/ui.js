 ...
 * @param {object} opts
 */
export function resizeWindow(title, {w="max-content", h="max-content"} = {}) {
    let windows = d.getElementsByClassName("css-xxceif");
    // Find the window with this title
    let window;
    for (let n=0; n<windows.length; n++) {
        if (String(windows[n].innerText).includes(title)) {
            window = windows[n];
            break;
        }
    } 
    ...
    // Go up until we find the window, then down to the part that's resizable
    window = window.parentElement.parentElement.parentElement.getElementsByClassName("react-resizable")[0];
    if (Number(w)) {
        w = String(1.2*w) + "em";
    }
    if (Number(h)) {
    ...
    if (w !== null) {
        window.style.setProperty("width", w);
    }
    if (h !== null) {
        window.style.setProperty("height", h);
    }
    return true;
}
