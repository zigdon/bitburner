...
export async function newUI(ns, id, label, opts={timeout: 10000}) {
    ...
    return {
        ...
        remove: async t => await ns.writePort(ports.UI, `remove ${id}`),
    };
}
...
export function hideCustomOverview(id) {
    const row = document.getElementById("z"+id);
    if (!row) {
        return;
    }
    row.hidden = true;
...
export function customOverview(id, name) {
    ...
    if (!(shareRow = document.getElementById("z"+id))) {
        ...
        const hook = document.getElementById("overview-extra-hook-2");
...
export function setCustomOverview(id, val) {
    ...
    const cell = document.getElementById("z"+id+"Val");
    val.replaceAll("\n", "<br>");
    cell.innerText = val;
}
