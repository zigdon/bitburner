...
export async function buyHardware(ns, name, cities, order) {
  ...
  for (let city of cities) {
    for (let [m, q] of Object.entries(order)) {
      if (missing[city] === undefined) { missing[city] = {}; }
      let cur = ns.corporation.getMaterial(name, city, m).qty;
      if (cur < q) {
        missing[city][m] = q-cur;
        await log("%s: Missing %s %s", city, fmt.large(missing[city][m]), m);
      }
    }
  }

  for (let [city, orders] of Object.entries(missing)) {
    for (let [m, q] of Object.entries(orders)) {
      ns.corporation.buyMaterial(name, city, m, q/10);
    }
  }
  await waitForTick(ns);
  for (let [city, orders] of Object.entries(missing)) {
    for (let m of Object.keys(orders)) {
 ...
 * @param {number} round
 */
export async function getInvestment(ns, amt, round) {
  let log = async (t, ...a) => await netLog(ns, t, ...a);
  ...
  await out("Waiting for a round %d investment of %s+...", round, fmt.money(amt));
  let tryFraud = true;
  while (true) {
    ...
    if (tryFraud) {
      ...
      await log("Building up stock, waiting 3 ticks...")
      await waitForTick(ns);
      await log("Building up stock, waiting 2 ticks...")
      await waitForTick(ns);
      await log("Building up stock, waiting 1 tick...")
      await waitForTick(ns);
